...
public class Cube extends Entity implements IPhysicEntity, ISingleEntityBlock {
    ...
    public Cube() {
        ...
        state.set(BlockState.getInstance("base:grass[default]"));
...
//            body.setPhysicsRotation(rotation = new Quaternion());
    ...
    public void render(Camera worldCamera) {
        ChunkMeta meta = PhysicsThread.chunkMap.get(InGame.getLocalPlayer().getZone(InGame.world).getChunkAtPosition(position));
        if (meta != null)
