...
public class LongThread {
    ...
    private static Structure getStructureAtVec(Map<Vec3i, Structure> structureMap, Vec3i vec3i) {
        ...
        Structure structure = structureMap.get(new Vec3i(cx, cy, cz));
        return structure == null ? emptyStructure : structure;
    }

    private static BlockState getBlockState(Map<Vec3i, Structure> structureMap, Pair<Vec3i, Structure> a, int x, int y, int z) {
    ...
    private static BlockState getBlockState(Map<Vec3i, Structure> structureMap, Pair<Vec3i, Structure> a, Pair<Vec3i, Structure> b, int x, int y, int z) {
        ...
        if (a != null && a.getLeft().x() == cx && a.getLeft().y() == cy && a.getLeft().z() == cz) {
            c = a.getRight();
        } else if (b != null && b.getLeft().z() == cx && b.getLeft().y() == cy && b.getLeft().z() == cz) {
            c = b.getRight();
        } else {
            c = getStructureAtVec(structureMap, new Vec3i(x, y, z));
        }
    ...
    private static Array<MeshData> getMeshData(Map<Vec3i, Structure> structureMap, Vec3i pos, Structure chunk) {
                    ...
                    if ((isEntirelyOpaque || isEntirelyOneBlockSelfCulling) && localX != 0 && localX != 15 && localY != 0 && localY != 15 && localZ != 0 && localZ != 15) {
                        ...
                        if (b != airBlockState) {
                            ...
                            if (completeCullMask != 63) {
                                ...
                                BlockState bnxnynz = getBlockState(structureMap, structurePair, globalX - 1, globalY - 1, globalZ - 1);
                                BlockState bnxny0z = getBlockState(structureMap, structurePair, globalX - 1, globalY - 1, globalZ);
                                BlockState bnxnypz = getBlockState(structureMap, structurePair, globalX - 1, globalY - 1, globalZ + 1);
                                BlockState bnx0ynz = getBlockState(structureMap, structurePair, globalX - 1, globalY, globalZ - 1);
                                BlockState bnx0ypz = getBlockState(structureMap, structurePair, globalX - 1, globalY, globalZ + 1);
                                BlockState bnxpynz = getBlockState(structureMap, structurePair, globalX - 1, globalY + 1, globalZ - 1);
                                BlockState bnxpy0z = getBlockState(structureMap, structurePair, globalX - 1, globalY + 1, globalZ);
                                BlockState bnxpypz = getBlockState(structureMap, structurePair, globalX - 1, globalY + 1, globalZ + 1);
                                BlockState b0xnynz = getBlockState(structureMap, structurePair, globalX, globalY - 1, globalZ - 1);
                                BlockState b0xnypz = getBlockState(structureMap, structurePair, globalX, globalY - 1, globalZ + 1);
                                BlockState b0xpynz = getBlockState(structureMap, structurePair, globalX, globalY + 1, globalZ - 1);
                                BlockState b0xpypz = getBlockState(structureMap, structurePair, globalX, globalY + 1, globalZ + 1);
                                BlockState bpxnynz = getBlockState(structureMap, structurePair, globalX + 1, globalY - 1, globalZ - 1);
                                BlockState bpxny0z = getBlockState(structureMap, structurePair, globalX + 1, globalY - 1, globalZ);
                                BlockState bpxnypz = getBlockState(structureMap, structurePair, globalX + 1, globalY - 1, globalZ + 1);
                                BlockState bpx0ynz = getBlockState(structureMap, structurePair, globalX + 1, globalY, globalZ - 1);
                                BlockState bpx0ypz = getBlockState(structureMap, structurePair, globalX + 1, globalY, globalZ + 1);
                                BlockState bpxpynz = getBlockState(structureMap, structurePair, globalX + 1, globalY + 1, globalZ - 1);
                                BlockState bpxpy0z = getBlockState(structureMap, structurePair, globalX + 1, globalY + 1, globalZ);
                                BlockState bpxpypz = getBlockState(structureMap, structurePair, globalX + 1, globalY + 1, globalZ + 1);
                                ...
                                if (md == null) {
//                                    md = new MeshData(shader, renderOrder);
    ...
    public boolean hasChunksToMesh() {
        return !this.runnable.runners.isEmpty();
    }
